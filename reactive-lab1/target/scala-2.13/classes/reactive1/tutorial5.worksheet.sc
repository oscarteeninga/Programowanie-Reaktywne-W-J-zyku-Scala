...
class Rational(val numer: Int, val denom: Int = 1) {
  def size = Math.sqrt(numer*numer+denom*denom)
...
object Rational {
  ...
  implicit val orderingRational: Ordering[Rational] = (x: Rational, y: Rational) =>
...
object Basket {
  ...
  implicit val ordBasket: Ordering[Basket] = (x: Basket, y: Basket) =>
    x.items.size.compareTo(y.items.size)
}
